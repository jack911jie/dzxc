...
class LegoPics:
    def __init__(self,crsDate,crsName,place_input='001-超智幼儿园',weekday=2,term='2020秋',mode=''):
        ...
        self.stu_sigDir=self.stu_sigDir.replace('$',place_input).replace('year',term[:4])
    ...
    def read_sig(self,weekday):
        ...
        if self.mode=='tiyan':
            sigFile=os.path.join(self.std_sig_dir,self.place,'学生信息表',self.term[:4],self.term+'-学生信息表（体验）.xlsx')
        else:
            sigFile=os.path.join(self.std_sig_dir,self.place,'学生信息表',self.term[:4],self.term+'-学生信息表（周'+wd+'）.xlsx')
